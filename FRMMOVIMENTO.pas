...
type
  Tformmov = class(TForm)
    ...
    procedure CarregarMovimentohistoricoEExibir;
  private
...
begin
  ...
  if Assigned(frmmenu.labelproduto) then
    LabelProd.Caption := frmmenu.labelproduto.Caption
  else
...
procedure Tformmov.CarregarMovimentohistoricoEExibir;
var
  sCodInterno: string;
begin
  // 1. Validação e Captura do CodInterno do formulário principal (frmmenu)
  ...
  sCodInterno := Trim(frmmenu.DBText45.caption);

  // 1.5. CAPTURA O NOME DO PRODUTO E EXIBE NA LABEL DO NOVO FORMULÁRIO
  if Assigned(frmmenu.DBText14) then
    LabelProd.Caption := frmmenu.DBText14.Caption
  else
    LabelProd.Caption := 'Produto não carregado';
