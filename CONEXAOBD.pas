...
var
  PrecoCusto: Currency;
  Diferenca: Real; // ⭐ CRÍTICO: Usar Real (ou Extended) para ler ftFloat
begin
  // 1. Verificação de segurança: Evita erros se os campos estiverem nulos
  ...
  // 2. Coleta os valores. PrecoCusto é Currency.
  PrecoCusto := DataSet.FieldByName('PrecoCusto').AsCurrency;

  // 3. Coleta o valor da diferença de quantidade como Real (AsFloat)
  Diferenca := DataSet.FieldByName('Diferenca').AsFloat;

  // 4. Calcula o Valor Diferença (Custo Unitário * Diferença de Quantidade)
